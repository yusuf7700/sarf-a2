...
// ===== Boblar ro'yxati (1-bob tayyor, qolganlari tez orada) =====
  ...
  window.answerQuiz = function(i){
    ...
    if(correct) s.score++;
    setQStatus(q.id, correct ? 'bilgan' : 'bilmagan');
    setTimeout(() => {
      s.current++;
      s.answered = false;
      renderQuizQuestion();
  ...
  function renderSozlamalar(){
    const html = `
      ...
      <div class="card">
        <h3>${L('Ilova')}</h3>
        <p style="font-size:12.5px;color:var(--muted);margin:0 0 10px;">${L("Yangilanish ko'rinmayotgan bo'lsa yoki ilova g'alati ishlayotgan bo'lsa, keshni tozalab qayta yuklang.")}</p>
        <div id="clearCacheBtn" onclick="clearAppCache()"
          style="border:1.5px solid #c65959;border-radius:10px;padding:10px 14px;cursor:pointer;font-size:13px;font-weight:700;color:#c65959;text-align:center;">
          🗑️ ${L('Keshni tozalash')}
        </div>
      </div>`;
