...
// ===== Boblar ro'yxati (1-bob tayyor, qolganlari tez orada) =====
  ...
  function pullStatusFromCloud(){
    if(!currentUser || typeof db === 'undefined') return Promise.resolve();
  ...
  window.loginGoogle = function(){
    if(typeof auth === 'undefined'){ alert(L("Internet aloqasi yo'q, keyinroq urinib ko'ring")); return; }
  ...
  window.loginAnonStart = function(){
    ...
    setTimeout(() => { const inp = document.getElementById('anonNameInput'); if(inp) inp.focus(); }, 50);
  };

  window.loginAnonConfirm = function(){
    if(typeof auth === 'undefined'){ alert(L("Internet aloqasi yo'q, keyinroq u
